...
int playTicTacToe(SOCKET s, std::string serverName, std::string remoteIP, std::string remotePort, int localPlayer)
{
	...
	while (winner == noWinner) {
		char port[MAX_SEND_BUF];
		char IP[MAX_SEND_BUF];
...
****/
			...
			_itoa_s(move, buffer, 10);

			strcpy_s(port, remotePort.c_str());
			strcpy_s(IP, remoteIP.c_str());

			int charsSent = UDP_send(s, buffer, strlen(buffer)+1, IP, port);
...
****/
				...
				int charsRecd = UDP_recv(s, buffer, sizeof(buffer), IP, port);
				move = atoi(buffer);
